...
public class ApiPolicyApplication extends Application<ApiPolicyConfiguration> {
  ...
  public void run(final ApiPolicyConfiguration configuration, final Environment environment) {
    ...
    Set<KnownEndpoint> knownWildcardEndpoints =
        getKnownWildcardEndpoints(configuration, environment, apiFilters);
    Set<KnownEndpoint> knownWhitelistedPostEndpoints =
        getKnownWhitelistedPostEndpoints(configuration, environment);

    environment
        .jersey()
        .register(
            new WildcardEndpointResource(
                new RequestHandler(new MutableHttpTranslator(), knownWildcardEndpoints),
    ...
    environment
        .healthChecks()
        .register(
            "Reader API Connectivity",
            new ReaderNodesHealthCheck(
                "Reader API Connectivity",
                configuration.getVarnish(),
                healthcheckClient,
                configuration.isCheckingVulcanHealth()));
  ...
  private Set<KnownEndpoint> getKnownWhitelistedPostEndpoints(
      ApiPolicyConfiguration configuration, Environment environment) {
  ...
  private Set<KnownEndpoint> getKnownWildcardEndpoints(
      ApiPolicyConfiguration configuration, Environment environment, ApiFilters apiFilters) {
    Set<KnownEndpoint> knownWildcardEndpoints = new LinkedHashSet<>();

    knownWildcardEndpoints.add(createEndpoint(environment, configuration, "^/things.*", "things"));
    ...
    knownWildcardEndpoints.add(
        createEndpoint(
            environment, configuration, "^/lists.*", "lists", apiFilters.listsFilters()));
    knownWildcardEndpoints.add(
        createEndpoint(
            environment,
            configuration,
            "^/internalcontent/.*",
            "internalcontent",
            apiFilters.internalContentFilters()));
    knownWildcardEndpoints.add(
        createEndpoint(
            environment,
            configuration,
            "^/internalcontent-preview/.*",
            "internalcontent-preview",
            apiFilters.internalContentPreviewFilters()));
    knownWildcardEndpoints.add(
        createEndpoint(
            environment,
            configuration,
            "^/enrichedcontent/.*",
            "enrichedcontent",
            apiFilters.enrichedContentFilters()));
    knownWildcardEndpoints.add(
        createEndpoint(
            environment,
            configuration,
    ...
    knownWildcardEndpoints.add(
        createEndpoint(
            environment,
            configuration,
            "^/content/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}/annotations",
            "public-annotations-api"));
    // identifiersFilter needs to be added before webUrlAdder & canonicalWebUrlAdder in the pipeline
    // since webUrlAdder's & canonicalWebUrlAdder's logic is based on the json property that
    // identifiersFilter might remove
    knownWildcardEndpoints.add(
        createEndpoint(
            environment,
            configuration,
            "^/content/.*",
            "content",
            apiFilters.contentIdentifiersFilters()));
    knownWildcardEndpoints.add(
        createEndpoint(
            environment,
            configuration,
  ...
  private KnownEndpoint createEndpoint(
      Environment environment,
      ApiPolicyConfiguration configuration,
      String urlPattern,
      String instanceName,
      ApiFilter... filterChain) {
    final Client client =
        JerseyClientBuilder.newBuilder()
            .property(ClientProperties.FOLLOW_REDIRECTS, false)
            .property(
                ClientProperties.CONNECT_TIMEOUT,
                configuration.getVarnish().getConnectTimeoutMillis())
            .property(
                ClientProperties.READ_TIMEOUT, configuration.getVarnish().getReadTimeoutMillis())
            .build();

    final RequestForwarder requestForwarder =
        new JerseyRequestForwarder(client, configuration.getVarnish());
