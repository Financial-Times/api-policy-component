...
public class AddSyndicationTest {
  ...
  private static byte[] readFileBytes(final String path) {
    try {
      return Files.readAllBytes(
          Paths.get(AddSyndicationTest.class.getClassLoader().getResource(path).toURI()));
  ...
  public void shouldNotProcessNotJSON() {
    ...
    final MutableResponse response =
        new MutableResponse(
            new MultivaluedHashMap<>(), responseBody.getBytes(Charset.forName("UTF-8")));
  ...
  public void shouldHaveOriginalFieldIfPresentAndPolicy() throws IOException {
    ...
    final MutableResponse initialResponse =
        new MutableResponse(
            new MultivaluedHashMap<>(), readFileBytes("sample-article-with-image.json"));
  ...
  public void shouldHaveFieldAddedIfMissingRegardlessPolicy() throws IOException {
    ...
    final MutableResponse initialResponse =
        new MutableResponse(
            new MultivaluedHashMap<>(), readFileBytes("sample-article-no-canBeSyndicated.json"));
    final MutableResponse expectedResponse =
        new MutableResponse(
            new MultivaluedHashMap<>(), readFileBytes("sample-article-with-image.json"));
    initialResponse.setStatus(200);
