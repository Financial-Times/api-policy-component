...
public class FluentLoggingBuilderTest {
                ...
                new ArgumentMatcher() {
                  ...
                  public boolean matches(final Object argument) {
                    ...
                    return containsBasicJSONFields(content)
                        && containsFieldInJSON(
                            "\"uuid\":\"7398d82a-6e76-11dd-a80a-0000779fd18c\"", content)
                        && containsFieldInJSON(
                            "\"transaction_id\":\"transaction_id=tid_test1\"", content)
  ...
  public void logVerifyAllOutboundFieldsArePresent() throws Exception {
    ...
    log.withResponse(resp).withUri(fakeURI).build().logDebug();
                ...
                new ArgumentMatcher() {
                  ...
                  public boolean matches(final Object argument) {
                    ...
                    return containsBasicJSONFields(content)
                        ...
                        && containsFieldInJSON("\"logLevel\":\"DEBUG\"", content)
                        && containsFieldInJSON(
                            "\"user-agent\":\"" + APPLICATION_NAME + " (+" + RUNBOOK_URI + ")\"",
                            content)
                        && containsFieldInJSON("\"accept\":\"application/json\"", content)
                        && containsFieldInJSON(
                            "\"content-type\":\"application/json;application/xml\"", content)
  ...
  public void logVerifyAllInboundFieldsArePresent() throws Exception {
    ...
    when(mockHttpHeaders.getHeaderString(mockHttpHeaders.ACCEPT))
        .thenReturn(APPLICATION_JSON_TYPE.toString());
    when(mockHttpHeaders.getHeaderString(mockHttpHeaders.USER_AGENT)).thenReturn("test user-agent");

    log.withRequest(req).withUriInfo(mockUI).withInboundHeaders(mockHttpHeaders).build().logDebug();
  ...
  public void logVerifyAllExceptionFieldsArePresent() {
    ...
    log.withException(ex).build().logDebug();
                ...
                new ArgumentMatcher() {
                  ...
                  public boolean matches(final Object argument) {
                    ...
                    return containsBasicJSONFields(content)
                        && containsFieldInJSON("\"logLevel\":\"DEBUG\"", content)
                        && containsFieldInJSON(
                            "\"exception_message\":\"Exception was null\"", content)
                        ...
                        && !containsFieldInJSON("\"method\":\"GET\"", content)
                        && !containsFieldInJSON(
                            "\"uri\":\"http://localhost:8080/content\"", content)
                        && !containsFieldInJSON("\"path\":\"/content\"", content)
                        && !containsFieldInJSON(
                            "\"user-agent\":\"" + APPLICATION_NAME + " (+" + RUNBOOK_URI + ")\"",
                            content)
