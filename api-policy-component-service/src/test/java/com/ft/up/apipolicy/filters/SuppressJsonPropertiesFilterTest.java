...
public class SuppressJsonPropertiesFilterTest {
  ...
  public static final DropwizardAppRule<ApiPolicyConfiguration> RULE =
      new DropwizardAppRule<>(
          ApiPolicyApplication.class, ResourceHelpers.resourceFilePath("config-junit.yml"));

  private static final String LAST_MODIFIED = "lastModified";
  private static final ObjectMapper MAPPER = new ObjectMapper();

  private final JsonConverter jsonConverter = new JsonConverter(MAPPER);
  private SuppressJsonPropertiesFilter removeJsonPropertiesUnlessPolicyPresentFilter =
      new SuppressJsonPropertiesFilter(jsonConverter, "comments", "lastModified");
  private static ApiFilters apiFilters = new ApiFilters();

  @Mock private MutableRequest mockedRequest;

  @Mock private HttpPipelineChain mockedChain;
  ...
  private static byte[] readFileBytes(final String path) {
    try {
      Path resourceFilePath =
          Paths.get(
              SuppressJsonPropertiesFilterTest.class.getClassLoader().getResource(path).toURI());
  ...
  public void testFiltersJsonProperties() throws Exception {
    ...
    final MutableResponse initialResponse =
        new MutableResponse(headers, readFileBytes("sample-article-with-image.json"));
    initialResponse.setStatus(200);
    when(mockedChain.callNextFilter(mockedRequest)).thenReturn(initialResponse);

    final MutableResponse processedResponse =
  ...
  public void testListResponse() throws IOException {
    ...
    headers.add("Content-Type", MediaType.APPLICATION_JSON);
    final MutableResponse initialResponse =
        new MutableResponse(headers, readFileBytes("sample-list-response.json"));
    final MutableResponse expectedResponse =
        new MutableResponse(headers, readFileBytes("sample-list-response-processed.json"));
    initialResponse.setStatus(200);
    when(mockedChain.callNextFilter(mockedRequest)).thenReturn(initialResponse);
